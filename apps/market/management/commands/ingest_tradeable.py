...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--failed-only",
            action="store_true",
            help="Retry only instruments whose latest Yahoo daily ingestion failed.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["failed_only"]:
            queryset = queryset.filter(
                ingestion_states__provider="yahoo",
        ...
        self.stdout.write(f"{mode.upper()}: {len(instruments)} Tradeable instruments")

        for position, instrument in enumerate(instruments, start=1):
            provider_code = yahoo_symbol(instrument)
            state, _ = OHLCVIngestionState.objects.get_or_create(symbol=instrument)
